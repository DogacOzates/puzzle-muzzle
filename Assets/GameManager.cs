...
public class GameManager : MonoBehaviour
{
    ...
    private void AdjustCameraToGrid()
    {
        ...
        const float marginFrac = 0.10f; // visual breathing room above and below the grid
        float uiFrac = topInsetFrac + botInsetFrac + topBarFrac + botBarFrac;
        float gameAreaFrac = Mathf.Max(1f - uiFrac - 2f * marginFrac, 0.38f);

        // Horizontal margin fraction on each side
        const float horzMarginFrac = 0.10f;
