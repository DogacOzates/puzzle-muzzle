...
public enum CellShape { Square, Pentagon, Hexagon, SevenGen }
    ...
    public static LevelData[] Levels
    {
        get
        {
            if (_levels == null)
            {
                ...
                LevelData[] sevenGenLevels = LevelGenerator.GenerateSevenGenCampaign(300);
                for (int i = 0; i < 300; i++)
                    _levels[900 + i] = sevenGenLevels[i];
            }
            return _levels;
        }
    }
