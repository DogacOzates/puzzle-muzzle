...
public static class LevelGenerator
{
        ...
        int backTrim)
    {
        ...
        for (int start = frontTrim + 1; start + blockedLength < playableEndExclusive; start++)
        {
            ...
            for (int i = start; i <= end; i++)
            {
                ...
                int depth = Mathf.Min(Mathf.Min(cell.x, width - 1 - cell.x), Mathf.Min(cell.y, height - 1 - cell.y));
                depthSum += depth;
                if (depth > 0)
                    interiorCount++;
            }
        ...
        for (int interiorBlockedLength = Mathf.Min(desiredBlocked, 8); interiorBlockedLength >= 2; interiorBlockedLength--)
        {
            ...
            for (int frontTrim = 0; frontTrim <= edgeTrimCount; frontTrim++)
            {
                ...
                foreach (TriangleRemovableWindow window in windows)
                {
                    ...
                    float score = window.depthSum * 30f
                        + window.interiorCount * 30f
                        - edgeTrimCount * 18f
    ...
    private static float ScoreTriangleWindowCombination(List<TriangleRemovableWindow> windows, int edgeTrimCount)
    {
        ...
        for (int i = 0; i < windows.Count; i++)
        {
            ...
            float center = (window.startIndex + window.endIndex) * 0.5f;
            score += window.depthSum * 30f;
            score += window.interiorCount * 30f;
            score += window.regionBonus;
        ...
        int levelIndex = 0)
    {
        ...
        // Cycle through column pairs so different levels prefer different grid regions
        int columnPairs = Mathf.Max(1, config.width / 2 - 1);
        int preferredColPair = levelIndex % columnPairs;
        float preferredX = (preferredColPair * 2 + 2) + 0.5f;  // center of the preferred column pair
        ...
        foreach (int edgeTrimCount in edgeTrimOptions)
        {
            ...
            for (int frontTrim = 0; frontTrim <= edgeTrimCount; frontTrim++)
            {
                ...
                // Apply column-pair region bonus so different levels prefer different grid sections
                for (int wi = 0; wi < windows.Count; wi++)
                {
                    TriangleRemovableWindow w = windows[wi];
                    float avgX = 0f;
                    for (int j = w.startIndex; j <= w.endIndex; j++)
                        avgX += fullPath[j].x;
                    avgX /= (w.endIndex - w.startIndex + 1);
                    w.regionBonus = Mathf.Abs(avgX - preferredX) <= 1.5f ? 50f : 0f;
                    windows[wi] = w;
                }
